...
class NeuraNet{
	...
	_initW(){
		...
		g=new Array(6);
		g[0]=[0.005, 0.005, 5.09671984640895, 0.005, 0.005, 0.005]
		g[1]=[0.005, 5.056632651164252, 0.4556897712266267, 0.005, 0.005, 0.005]
		g[2]=[0.6411984514887498, -27.221662247793784, 7.163020754837719, 0.005, 0.005, 0.005]
		g[3]=[0.005, 22.96495972864052, 12.413427940155941, 0.005, 0.005, 0.005]
		g[4]=[118.00291942227723, -26.743336653406054, 261.5917644999202, 0.7130328716572676, 0.6019998510044426]
		g[5]=[135.21312305572036, 255.23703234089592, -5.598937185302354, 8.907411720962195, 11.783660406007813]
		this.netWs.push(g);
		
		g=new Array(6);
		g[0]=[0.005, 3.0619952887383404, -0.07395417971094538, 0.005, 0.14489151975361472, 0.1447047063267849]
		g[1]=[-0.7563075235176355, 7.307524961677446, -0.5611283353397043, 2.2335667274731956, -1.0357334386604504, -18.01622041180993]
		g[2]=[0.4648842240007985, -28.110536825472483, 3.842838508568969, 0.005, 1.0140978721456873, 7.607249885268982]
		g[3]=[0.005, 0.005, 3.8133281254059694, -0.13244401090461116, 0.21217332357331298, 0.21272539539083232]
		g[4]=[250.73110464110664, 431.2878681183944, 26.58849292325006, 13.035727384834175, 7.429870717681749]
		g[5]=[177.38164577372194, 39.97677939188287, -18.707454940803718, 14.153663646099252, 357.60253063396567]
		this.netWs.push(g);
		
		g=new Array(6);
		g[0]=[0.005, 2.5459322364070576, 0.714342204175392, -0.27894046901980774, 0.28366263932045743, 0.053659694693780485]
		g[1]=[0.1327663217154489, -1.830525753658706, 3.7808894994196596, -0.2698340843834473, 0.18770728541939377, 0.5906557832193832]
		g[2]=[-4.342540995385476, -3.9658304743668467, 4.481919112064883, 0.7552147685541966, 13.005585588876801, 1.4948308907046655]
		g[3]=[1.5038935923786556, -2.07855837467848, 59.02771748592528, -0.19808911349677916, 6.261140150855489, -3.1000812520705217]
		g[4]=[180.0432519324972, 454.2494919623617, -178.3082536885722, 44.733887970993116, 42.38232888369051]
		g[5]=[173.5305851257766, 200.3518658649381, 293.93430112107563, -29.313646227734544, -0.4859906184439732]
		this.netWs.push(g);
		/*
		g=new Array(6);
	...
	_showAndsetW(){
		//显示到#netupdata
		$("#netupdata>select>option").remove();	
		...
		//设置权值
		this._setWWs(this.netWs[this.netWs.length-1]);
	}
	...
	teach3(){
		let worker = new Worker('scripts/engine/ai/state/NetF.js');
		// alert("开辟另一线程更新权值中，请稍等");
		console.log("开辟另一线程更新权值中，请稍等");
		var sTime=new Date().getTime();
		this._showtime2();
		worker.postMessage(this.TT);	
		var kkself=this;
		worker.addEventListener('message', function(e){
			if(e.data.method=="tip"){
				// console.log("进度条：");
				// console.log(e.data.reply);
				kkself._uptime(e.data.reply[0],e.data.reply[1],e.data.reply[2]);
				
			}else if(e.data.method=="result"){
				kkself._closetime();
				console.log("经过"+(new Date().getTime()-sTime)/1000+"秒，修正了"+e.data.count+"次,开始E="+e.data.mE+",最终E="+e.data.E);
				// alert("经过"+(new Date().getTime()-sTime)/360+"分钟，修正了"+e.data.count+"次,最终E="+e.data.E);	
				//修改权值W
				let ggf=[];
				ggf[0]=e.data.reply[1][1].W;
				ggf[1]=e.data.reply[1][2].W;
				ggf[2]=e.data.reply[1][3].W;
				ggf[3]=e.data.reply[1][4].W;
				ggf[4]=e.data.reply[2][1].W;
				ggf[5]=e.data.reply[2][2].W;
				//添加
				kkself.netWs.push(ggf);
				//显示并设置netWs的最后一名
				kkself._showAndsetW();
				//再次设置ggf
				kkself._setWWs(ggf);
	...
	_uptime(ne,nt,me){
		...
		p2.children("div").css("width",p2[0].clientWidth*nt/2500);
		p2.children("span").eq(0).text("Count比例："+nt+"/2500");
